 ...
 */
class Upload {
	var $max_upload_size = "8388608";	//max upload file byte (8M)
	...
	function multiSave($field, $dir, $postid) {
		...
		if (is_array($field_names)) {
			while(list($key, $file_name) = each($field_names)) {
				...
				//echo $file_name."<br>".split(".",$file_name);
				list($nameF, $typeF) = split("[.]",$file_name,2);
				$file_name= $nameF."_".$postid."_".$key.".".$typeF;
