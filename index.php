	...
	{ 
		switch ($_REQUEST["action"]) 
		{
			...
			case 'send_email':
							...
							$msg = "<p><img src=\"http://www.indianfoodsonline.co.uk/mail/logo.jpg\" /></p>";
			...
			case 'registration':
								...
								if($tecncode==$oencode)
								{
									$query = "INSERT INTO  userregistration( `id` , `first_name` , `last_name` , `phone` , `mobile` , `fax` , `email` , `address1` , `password` , `status` ) VALUES ('', '".
								    ...
								    $queryr = "INSERT INTO  resturant( `rid` , `userid` , `area_id` , `chinise_id` , `rname` , `street` , `city` , `zipcode` , `country_id` , `contact` , `contact2` , `contact3` , `contact4` , `web` , `logo` , `rdesc` , `profile` , `keytitle` , `aactive` , `rating` )  VALUES ('','$postid','','','".mysql_escape_string($_REQUEST["rname"])."','','','','','','','','','','','','','','','')";
			...
			case 'menu_list':
							...
							if($_SESSION['rids']!="")
							{
								...
								//$_SESSION['order_type']= ($_SESSION['order_type'] !="")?$_SESSION['order_type']:1;
			...
			case 'menu_list_collection':
										...
										if($_SESSION['rids']!="")
										{
											$_SESSION['order_type']= ($_SESSION['order_type'] > 1)?$_SESSION['order_type']:1;
											$arreturnvalue = array('body' => 'order_menu.html');
										}
										else
										{
			...
			case 'insert_new_menue':
								 	if($_SESSION['userid']!="")
									{
								        ...
								        if (!$success) 
									    {
											$query = "INSERT INTO categories( `categories_id` , `rid` , `categories_name` , `short_desc` , `parent_id` , `sort_order` , `date_added` , `last_modified` ) VALUES('','$rids','".mysql_escape_string($_REQUEST["categories_name"])."','','".mysql_escape_string($_REQUEST["parent_id"])."','','$d','')";
											$result = $db->execQuery($query);
			...
			case 'insert_reservation':
									...
									if($_SESSION['rids']!="")
									{
										...
										$_SESSION['aritimes']=date('H:i', strtotime($atimes));
										$dayt=$order_date;
										$dayt = strtotime($dayt);
										$daytr = date('l', $dayt);
			...
			case 'customer_login':
								...
								{
									$message = ($cloginstatus != false)?($cloginstatus == 's')?"Account has been suspended!":"Account is pending!":"Invalid Username / Password!";
									if($_REQUEST['otp']=="orp")
									{					
										$pageName='My Account';
										$arreturnvalue = array(
...
// _sending mail
		...
		$message .= '<tr style="background:#FFF url(\'http://www.indianfoodsonline.co.uk/mail/logo_bg.gif\') repeat-x bottom;"><td colspan="2" style="padding:0px;color:#000000;" align="right"><img style="float:left;" src="http://www.indianfoodsonline.co.uk/mail/logo.jpg" /><h2>Account Information</h2> <div style="background-color:#fff;font-size:11px;text-align:right;">'. date('d/m/Y H:i:s A') .'&nbsp; </div></td></tr>';
		...
		$message .= '<tr><td align="left" colspan="2"><br />Hello '.mysql_escape_string($_REQUEST["customers_firstname"]).' '.mysql_escape_string($_REQUEST["customers_lastname"]).', <p style="padding:0px 10px 20px 0px;margin:0px;"><br />Congratulations! You are now a registered Customer with Indian Foods Online. You can order your favorite Indian foods from your account and will be receiving various promotions and loyalty points as a registered Customer. Please leave your feed on every order you place with us so that we can constantly improve the service of our registered restaurants. Enjoy!</p></td></tr>';	
		$message .= '<tr><td align="left" colspan="2"><b>Your Account login information: </b></td></tr>';
		$message .= '<tr><td align="left" colspan="2">Your Email Address/UserID : '.mysql_escape_string($_REQUEST["customers_email_address"]). '</td></tr>';	
		$message .= '<tr><td align="left" colspan="2">Your Password  &nbsp; : '.mysql_escape_string($_REQUEST["customers_password"]). '</td></tr>';	
		...
		$send_to = mysql_escape_string($_REQUEST["customers_email_address"]); // reciever email	
		require_once("mail/htmlMimeMail5.php");
...
// mail sent to admin		
		$message = '<div style="border:1px solid #6a0004; background-color:#F4F2EE;width:700px;padding:0px 20px 0px 20px;"><div style="padding:20px;background-color:#FFFFFF;"><table width="100%" bgcolor="#FFFFFF" cellpadding="3" cellspacing="3" style="margin:0px auto;text-align:center; font-size:12px; font-weight:normal; font-family:verdana,arial;" border="0">';		
		$message .= '<tr style="background:#FFF url(\'http://www.indianfoodsonline.co.uk/mail/logo_bg.gif\') repeat-x bottom;"><td colspan="2" style="padding:0px;color:#000000;" align="right"><img style="float:left;" src="http://www.indianfoodsonline.co.uk/mail/logo.jpg" /><h2>New Customer Information</h2> <div style="background-color:#fff;font-size:11px;text-align:right;">'. date('d/m/Y H:i:s A') .'&nbsp; </div></td></tr>';
		$message .= '<tr><td align="left" colspan="2">&nbsp;</td></tr>';	
		$message .= '<tr><td align="left" colspan="2"><div style="color:#c73810; font-weight:bold; font-size:17px;">A new customer has recently signed up!</div></td></tr>';	
		
		
		$message .= '<tr><td align="left" colspan="2"><b>Customer Details</b></td></tr>';
		
		$message .= '<tr><td align="left" colspan="2">Name  &nbsp; : '.mysql_escape_string($_REQUEST["customers_firstname"]).' '.mysql_escape_string($_REQUEST["customers_lastname"]). '</td></tr>';	
		$message .= '<tr><td align="left" colspan="2">Email Address : '.mysql_escape_string($_REQUEST["customers_email_address"]). '</td></tr>';	
		$message .= '<tr><td align="left" colspan="2">Phone : '.mysql_escape_string($_REQUEST["customers_telephone"]). '</td></tr>';	
		$message .= '<tr><td align="left" colspan="2">Mobile : '.mysql_escape_string($_REQUEST["customers_fax"]). '</td></tr>';	
		$message .= '<tr><td align="left" colspan="2"><u>Adress :</u> </td></tr>';	
		$message .= '<tr><td align="left" colspan="2"><p>'.$_REQUEST["customers_address1"].' '.$_REQUEST["customers_address2"].'<br />'.$_REQUEST["customers_town"].', '.$_REQUEST["customers_state"].'<br />'.$_REQUEST["customers_postcode"].' '.$_REQUEST["customers_postcode2"].'<br />UK</p></td></tr>';	
		...
		$org = mysql_escape_string($_REQUEST["customers_firstname"]).' '.mysql_escape_string($_REQUEST["customers_lastname"]); // organization name
		$email = mysql_escape_string($_REQUEST["customers_email_address"]); // sender email
		$send_to = "info@indianfoodsonline.co.uk"; // reciever email	
...
// _end sending mail								
										...
										if($_SESSION[cart]!="")
										{
											...
											$_SESSION['customers_postcode']=mysql_escape_string($_REQUEST["customers_postcode"])." ".mysql_escape_string($_REQUEST["customers_postcode2"]);	
			...
			case 'insert_shipping_address':
											...
											else
											{
												...
												$_SESSION['customers_postcode']=$_REQUEST['customers_postcode']." ".$_REQUEST['customers_postcode2'];		
			...
			case 'reorder':
							...
							{		
								...
								{
								$_SESSION['order_type']=$rowor[0]["order_type"];
								}
								...
								//////////////////sagar///////////////////
								$otypevalue=0;
								$query="select * from resturant_policy where rid='".$_SESSION['rids']."'";
								$result = $db->execQuery($query);
								$otypeinfo = $db->resultArray($result);
								for($i=0;$i<sizeof($otypeinfo);$i++)
								{
									$otypevalue+=$otypeinfo[$i]['policy_id'];
								}
								$_SESSION['otypevalue']=$otypevalue;
								
								if($_SESSION['order_type']=="")
								{
									if($otypevalue==1 or $otypevalue>=3)
									{
										$_SESSION['order_type']=1;
									}
									elseif($otypevalue==2)
								...
								{
									if($_SESSION['otypevalue']==1 or $_SESSION['otypevalue']==2)
									{
										$_SESSION['order_type']=$_SESSION['otypevalue'];
									}
								}
			...
			case 'confirm_reservetion':
									  if($_SESSION['rids']!="")
									  {
											...
											{			
												$d=date('Y-m-d');
												$query="Insert into customer_regervetion( `resid` , `customers_id` , `rid` , `ar_date` , `ar_time` , `celebretion` , `nop` , `comments` , `rdate` , `rpstatus` , `gpay` ) values('', '".$_SESSION["customers_id"]."', '".$_SESSION["rids"]."', '".$_SESSION["arival_date"]."', '".$_SESSION["aritimes"]."', '".$_SESSION["celibration"]."', '".$_SESSION["nop"]."', '".$_SESSION['comments']."','$d','','')";
												$result = $db->execQuery($query);
												$postid = $db->lastInsert($result);
												$mlrid=$postid;
			...
			case 'confirm_orders': 
									...
									else
									{
											...
											/* CHANGED  BY MAHADEB _START
											if($_REQUEST['ortime']!="")
											{
												if($_REQUEST['fasth']==24) { $hours="00"; $order_dt=date( "Y-m-d", strtotime( "$order_date +1 day" ) ); } 
												elseif($_REQUEST['fasth']==25) { $hours="01"; $order_dt=date( "Y-m-d", strtotime( "$order_date +1 day" ) ); }
												elseif($_REQUEST['fasth']==26) { $hours="02"; $order_dt=date( "Y-m-d", strtotime( "$order_date +1 day" ) ); } 
												elseif($_REQUEST['fasth']==27) { $hours="03"; $order_dt=date( "Y-m-d", strtotime( "$order_date +1 day" ) ); } 
												else { $hours=$_REQUEST['fasth']; $order_dt=$order_date; }
												$mints=$_REQUEST['fastm'];
												$atimes="$hours:$mints";
												$stime=strtotime("$order_dt $atimes");
												$stime=date('Y-m-d H:i',$stime);
												$_SESSION['timeH']=$stime;
												$_SESSION['ortime'] = ($_REQUEST['ortime'] == "asap")?$_REQUEST['ortime']:"";
											}
											else
											{
												if($_REQUEST['hrs']==24) { $hours="00"; $order_dt=date( "Y-m-d", strtotime( "$order_date +1 day" ) ); } 
												elseif($_REQUEST['hrs']==25) { $hours="01"; $order_dt=date( "Y-m-d", strtotime( "$order_date +1 day" ) ); }
												elseif($_REQUEST['hrs']==26) { $hours="02"; $order_dt=date( "Y-m-d", strtotime( "$order_date +1 day" ) ); } 
												elseif($_REQUEST['hrs']==27) { $hours="03"; $order_dt=date( "Y-m-d", strtotime( "$order_date +1 day" ) ); } 
												else { $hours=$_REQUEST['hrs']; $order_dt=$order_date; }
												$mints=$_REQUEST['mints'];
												$atimes="$hours:$mints";
												$stime=strtotime("$order_dt $atimes");
												$stime=date('Y-m-d H:i',$stime);
											...
											*/
											...
											if($rowsed[$dayconls]!="Closed For the day" and $rowsed[$dayconds]!="Closed For the day")
											{
												...
												$atmd= $_REQUEST["delivery_time"]; //sdf
												
												if($atmd>=$lst and $atmd<=$len)
												{
													$ostatus=1; 
												}
												elseif($atmd>=$dst and $atmd<=$den)
											...
											elseif($rowsed[$dayconls]=="Closed For the day" and $rowsed[$dayconds]!="Closed For the day")
											{
												...
												// $atmd= strtotime("$order_dategt $atimes"); //has been changed
												$atmd= $_REQUEST["delivery_time"]; //sdf
												//echo date("Y-m-d H:i:s", $atmd).'>='.date("Y-m-d H:i:s", $dst).' and'. date("Y-m-d H:i:s", $atmd).'<='.date("Y-m-d H:i:s", $den); 
												if($atmd>=$dst and $atmd<=$den)
												{
													$ostatus=1;
												}
											...
											elseif($rowsed[$dayconls]!="Closed For the day" and $rowsed[$dayconds]=="Closed For the day")
											{
												...
												//$atm= strtotime($atm); // has been changed 
												$atm= $_REQUEST["delivery_time"]; //sdf
											...
											if($ostatus==1)
											{
												$_SESSION['customer_comments']= $_REQUEST['customer_comments'];
			...
			case 'insert_order':
											...
											else
											{
												...
												{
												 $d=date('Y-m-d H:i:s');
												 $query="Insert into customer_order( `orid` , `customers_id` , `rid` , `order_type` , `customers_address1` , `customers_address2` , `customers_town` , `customers_state` , `customers_country` , `customers_postcode` , `customer_comments` , `pay_mathod` , `pstatus` , `dstatus` , `com_fee` , `com_fee_status` , `hnd_fee` , `vat` , `ordate` , `delevery_time` , `ccfee`,`del_cost`, `order_status` ) values('', '".$_SESSION["customers_id"]."', '".$_SESSION["rids"]."','".$_SESSION['order_type']."', '".$_SESSION["customers_address1"]."', '".$_SESSION["customers_address2"]."', '".$_SESSION["customers_town"]."', '".$_SESSION["customers_state"]."', '".$_SESSION["customers_country"]."', '".$_SESSION["customers_postcode"]."', '".$_SESSION['customer_comments']."', '".$_SESSION["pay_mathod"]."', '','','','','".$_SESSION['hfee']."','','$d','".$_SESSION['timeH']."','".$_SESSION['ccfee']."','".$_SESSION['del_cost']."', '0')";	
													 ...
													 for ($i = 0; $i < sizeof($_SESSION[cart]); $i++) 
													 {
														...
														// $uprice=$rowp["price"];
														$uprice = ($rowp["price"] - discount_price($_SESSION['rids'], $rowp["categories_id"], $_SESSION['order_type'], $rowp["price"]));  // discount price has been deducted here.
														
														$queryod="Insert into order_detail( `oritemid` , `orid` , `product_id` , `qantity` , `uprice` ) values('', '$postid', '$pids', '$qtys','$uprice')";
...
///////////////////////////////////////////////////////////////////////////////New Mail send before reach paypal ///////////////////////////////////////
...
$resultpoint = $db->execQuery($sqlinpoint);
		...
		$sent = mail($send_to, $subject, $body_txt1, $headers); // mail has been sent to mobile via SMS
...
/////////////////////////
	...
	$body_txt .='</td>
				...
				<tr>
				  ...
				  <td valign="top" bgcolor="#FFFFFF" class="bodytext">Not yet paid';
	$body_txt .='</td>
...
////////////////////
	...
	$body_txt .=($arinfo[0]["order_type"]=="1")?" Delivery Address:":"Ordered From";
	...
	$body_txt .='</td>
		  ...
		  $body_txt .='</td>
						  ...
						  <td valign="top" bgcolor="#FFFFFF">';
		  $arinfo[0]["customers_country"] = ($arinfo[0]["customers_country"] == "")?"United Kingdom":$arinfo[0]["customers_country"];
		  $rowc["customers_country"] = ($rowc["customers_country"] == "")?"United Kingdom":$rowc["customers_country"];
		...
		$body_txt .='</td>
					...
					<tr>
					  <td align="left" valign="top" bgcolor="#FFFFFF">Mobile:</td>
					  <td valign="top" bgcolor="#FFFFFF">';
		$body_txt .= $rowc["customers_fax"];
		
		$body_txt .='</td>
					  <td colspan="2" rowspan="1" align="left"  valign="top" bgcolor="#FFFFFF">&nbsp;</td>
					</tr>
					<tr>
					  <td align="left" valign="top" bgcolor="#FFFFFF">Phone:</td>
					  <td valign="top" bgcolor="#FFFFFF">';
		$body_txt .= $rowc["customers_telephone"];
		...
		$body_txt .='</td>
					 <td colspan="2" rowspan="1" align="left"  valign="top" bgcolor="#FFFFFF">&nbsp;</td>
					  ...
					  <tr>
						<td align="center" height="1" bgcolor="#FFFFFF"></td>
					  </tr>
					  <tr>
		...
		while($rowpro=mysql_fetch_array($resultpro))
		{
			...
			$body_txt .=$rowpn["product_title"].'&nbsp;'.$rowpp["product_title"];
			$body_txt .='</td><td align="left" bgcolor="#FFFFFF">&pound;';
			$body_txt .=number_format($rowpro["uprice"], 2, '.', '');
			$body_txt .='</td><td align="center" bgcolor="#FFFFFF">';
			$body_txt .=$rowpro["qantity"];
			$body_txt .='</td><td align="left" bgcolor="#FFFFFF">&pound;';
			$stotal=$rowpro["uprice"]*$rowpro["qantity"];
			$body_txt .= number_format($stotal, 2, '.', '');
			$body_txt .='</td></tr>';
		  }
	  if($arinfo[0]['pay_mathod']=="Online")
	  {
	  $body_txt .='<tr>
					<td colspan="3" align="left" bgcolor="#FFFFFF"><strong>Credit Card Fee</strong></td>
					<td align="left" bgcolor="#FFFFFF"><b>&pound;';
	 $body_txt .=number_format($arinfo[0]["ccfee"], 2, '.', '');
	 $body_txt .='</b></td>
				  </tr>';
	 }
	 $body_txt .='<tr>
					...
					<td align="left" bgcolor="#FFFFFF"><b>&pound;';
	 $body_txt .=number_format($arinfo[0]["hnd_fee"], 2, '.', '');
   if ($arinfo[0]["order_type"]=="1")
   {
   $body_txt .='</b><td></tr>
				<tr>
				<td colspan="3" align="left" bgcolor="#FFFFFF"><strong>Delivery Cost</strong></td>
				<td align="left" bgcolor="#FFFFFF"><b>&pound;';
   $body_txt .=number_format($arinfo[0]["del_cost"], 2, '.', '');
   } 
	...
	$resulttpr=mysql_query($sqltpr);
	$rowprt=mysql_fetch_array($resulttpr);
	$ftotal=$rowprt["tprice"]+$arinfo[0]["ccfee"]+$arinfo[0]["hnd_fee"]+$arinfo[0]["del_cost"];
	$body_txt .=number_format($ftotal, 2, '.', '');
	$body_txt .='</b></td>
					  </tr>
							</table></td>
						  </tr>
						  ...
						  <tr>
							<td align="left" ><b>Customer Comments:</b></td>
						  </tr>
						  <tr>
							<td align="left"><div style=" border:1px solid #f0f0f0; width:100%;">';
			...
			case 'insert_order_roylty':
										...
										else
										{
											if($_SESSION['cart']!="")
											{
													...
													$d=date('Y-m-d H:i:s');
													 $query="Insert into customer_order( `orid` , `customers_id` , `rid` , `order_type` , `customers_address1` , `customers_address2` , `customers_town` , `customers_state` , `customers_country` , `customers_postcode` , `customer_comments` , `pay_mathod` , `pstatus` , `dstatus` , `com_fee` , `com_fee_status` , `hnd_fee` , `vat` , `ordate` , `delevery_time`) values('', '".$_SESSION["customers_id"]."', '".$_SESSION["rids"]."','".$_SESSION['order_type']."', '".$_SESSION["customers_address1"]."', '".$_SESSION["customers_address2"]."', '".$_SESSION["customers_town"]."', '".$_SESSION["customers_state"]."', '".$_SESSION["customers_country"]."', '".$_SESSION["customers_postcode"]."', '".$_SESSION['customer_comments']."', '".$_SESSION["pay_mathod"]."', '','','$fcommission','','$rhfees','".$_SESSION["vat"]."','$d','".$_SESSION['timeH']."')";	
													 $result = $db->execQuery($query);
							  						 $postid = $db->lastInsert($result);
													 $mlord=$postid;
													 for ($i = 0; $i < sizeof($_SESSION[cart]); $i++) 
...
/////////////////////////
												  ...
												  $body_txt .=$rowc["customers_address2"];
												  $body_txt .='</td>
												              <td align="left" valign="top" bgcolor="#FFFFFF"> City/Town</td>
												              <td valign="top" bgcolor="#FFFFFF">';
											      $body_txt .= $arinfo[0]["customers_town"];
												  $body_txt .='</td>
													            </tr>
												 ...
												 $body_txt .= $rowc["customers_country"];
												 $body_txt .='</td>
												              <td align="left"  valign="top" bgcolor="#FFFFFF"> County:</td>
												              <td valign="top" bgcolor="#FFFFFF">';
												 $body_txt .= $arinfo[0]["customers_state"];
												 $body_txt .='</td>
													            </tr>
												...
												$body_txt .='</td>
															  <td colspan="2" rowspan="2" align="left"  valign="top" bgcolor="#FFFFFF">&nbsp;</td>
															</tr>
															<tr>
															  <td align="left" valign="top" bgcolor="#FFFFFF">Phone:</td>
															  <td valign="top" bgcolor="#FFFFFF">';
											 ...
											 $body_txt .= number_format($arinfo[0]["vat"], 2, '.', '');
											 $body_txt .='</td>
									                      </tr>
									                      <tr>
									                        <td colspan="3" align="left" bgcolor="#FFFFFF"><strong>Handling Fees</strong></td>
...
/*											if(strlen($ordid)==1)
											...
											$sqlinpoint="insert into customer_roylpoint( `crid` , `customers_id` , `earn_rpoint` , `buy_rpoint` ) values('','$cuid','','$ebpt')";
											$resultpoint = $db->execQuery($sqlinpoint);
											$subject="";
											
											//////// sms message hard coded for instant use
											$body_txt1="A new order is placed, order NO: ".order_id($arinfo[0]["orid"]);
											////////////////////////////////////////////////////////////
								
											$sent = mail($send_to, $subject, $body_txt1, $headers); // this mail is sending by SMS gateway
									...
									# aql API Call start @Shafiq
											...
											$smsurl = "$weburl?username=$username&password=$password&to_num=$number&message=$message";
											 ...
											 // if $flash is set to 1, it will add the flash request onto the query
												...
												// if ($content_array = file($smsurl))	// temporarily skipped as SMS printer is not using
												if(false)																						
												  {
													    $content = implode("", $content_array); // skipped by mahadeb
			...
			case 'insert_order_cashdelivery':
											...
											else
											{
													...
													{
														 $d=date('Y-m-d H:i:s');
														 $query="Insert into customer_order( `orid` , `customers_id` , `rid` , `order_type` , `customers_address1` , `customers_address2` , `customers_town` , `customers_state` , `customers_country` , `customers_postcode` , `customer_comments` , `pay_mathod` , `pstatus` , `dstatus` , `com_fee` , `com_fee_status` , `hnd_fee` , `vat` , `ordate` , `delevery_time` , `ccfee`,`del_cost`) values('', '".$_SESSION["customers_id"]."', 
...
/////////////////////////
												    ...
												    $tt=explode(':',$dt[1]);
													$body_txt .= date('d/m/Y H:i:s',mktime($tt[0],$tt[1],$tt[2],$dd[1],$dd[2],$dd[0]));
													$body_txt .= ($arinfo[0]["order_type"]=="2")?($_SESSION["ortime"] == "asap")?"(ASAP)":"":""; 
													unset($_SESSION["ortime"]);
													$body_txt .='</td>
													                  <td width="23%" valign="top" bgcolor="#FFFFFF" class="bodytext">Restaurant Tel No: </td>
													                  <td width="29%" valign="top" bgcolor="#FFFFFF" class="bodytext">';
													$body_txt .=$roword["contact"];
...
////////////////////
													...
													$body_txt .='</td>
													 ...
													 $body_txt .='</td>
													  ...
													  $body_txt .='</td><td colspan="2" rowspan="1" align="left"  valign="top" bgcolor="#FFFFFF">';
													  $body_txt .='</td>
															            </tr>
															            <tr>
															              <td align="left" valign="top" bgcolor="#FFFFFF">Email</td>
													...
													while($rowpro=mysql_fetch_array($resultpro))
													{
													   ...
													   $body_txt .='</b></td>
														...
														$resulttpr=mysql_query($sqltpr);
														$rowprt=mysql_fetch_array($resulttpr);
														$ftotal=$rowprt["tprice"]+$arinfo[0]["hnd_fee"]+$arinfo[0]["del_cost"];
														$body_txt .= number_format($ftotal, 2, '.', '');
													...
													// restaurant owner mail
													$sqlc="Select userregistration.email from userregistration inner join resturant on userregistration.id=resturant.userid where rid ='".$_SESSION["rids"]."'";
													$resultc=mysql_query($sqlc);
													$rowc=mysql_fetch_array($resultc);
													$send_to = $rowc["email"];
...
*/													$body_txt1 .=order_id($ordid); 
													...
													////////////////////////////////////////////////////////////
														...
														$sent = mail($send_to, $subject, $body_txt1, $headers); // this email is sending  by SMS gateway
			...
			case 'insert_deliver_policy':
						 	if($_SESSION['userid']!="")
							{
						        ...
						        if ($_REQUEST["dpid"]=="") 
							    {
									 $query = "INSERT INTO delevary_policy( `dpid` , `rid` , `radious` , `take_time` , `del_time` , `min_order` ) VALUES('','$rids','".mysql_escape_string($_REQUEST["radious"])."','".mysql_escape_string($_REQUEST["take_time"])."','".mysql_escape_string($_REQUEST["del_time"])."','".mysql_escape_string($_REQUEST["min_order"])."')";
								     $result = $db->execQuery($query);
										  if ($result) 
										  {
							                    $arreturnvalue = array(
			...
			case 'insert_item':
								if($_SESSION['userid']!="")
								{
									...
									if($userfile_name!="")
									{
										...
										$query = "INSERT INTO product( `product_id` , `categories_id` , `product_title`, `thumb_image` , `large_image` , `product_desc` , `price`) VALUES('','".mysql_escape_string($_REQUEST["categories_id"])."','".mysql_escape_string($_REQUEST["product_title"])."','".strtolower($prod_img_thumb)."','".strtolower($prod_img_thumb1)."','".mysql_escape_string($_REQUEST["product_desc"])."','".mysql_escape_string($_REQUEST["price"])."')";
										$result = $db->execQuery($query);
      ...
      <!----- header + brade crum + menu+ baner start---->
	   ...
	   ?></div>
		...
		<!----- left content + right content  + mid content  start---->
		 <div id="middle"> 
		<? if($body=='order_menu.html' or $body=='restaurant_list.html' or $body=='reservetion.html' or $body=='confirmation.html')
		   {
